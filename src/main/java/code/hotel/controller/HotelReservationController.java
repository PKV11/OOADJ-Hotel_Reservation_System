...
public class HotelReservationController {
	...
	public String checkInReservation(@RequestParam("resId") int reservationId) {
		// Call the service method to update the status of the reservation to "checked
		// in"
		reservationService.checkInReservation(reservationId);
		return "redirect:/your-reservations"; // Redirect back to the reservations page
	}

	@PostMapping("/reservation-check-out")
	public String checkOutReservation(@RequestParam("resId") int resId) {
		// Call the service method to update the status of the reservation to "checked
		// out"
		reservationService.checkOutReservation(resId);
		return "redirect:/payment?resId=" + resId; // Redirect
	}

	// Payment page
	@GetMapping("/payment")
	public String showPaymentPage(@RequestParam("resId") int reservationId, Model model) {
		// Retrieve reservation details based on reservationId
		Reservation reservation = reservationService.getReservationForLoggedUserById(reservationId);

		// Add reservation details to the model
		model.addAttribute("reservation", reservation);
		// Add the price to the model
		model.addAttribute("price", reservation.getPrice());

		return "payment";
	}

	@PostMapping("/process-payment")
	public String processPayment(@RequestParam("resId") int resId) {
		// Call the service method to update the status of the reservation to "checked
		// out"
		reservationService.checkOutReservation(resId);
